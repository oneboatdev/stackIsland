...
+ 数组本身是_引用数据类型_，而数组中的元素可以是_任何数据类型_，包括基本数据类型和引用数据类型。
+ 创建数组对象会在内存中开辟出_一整块连续_的空间。占据的空间大小，取决于数组的长度和数组中元素的类型。
...
## 一维数组的初始化
### 静态初始化
如果数组变量的初始化和数组元素的赋值操作同时进行，就称为静态初始化。静态初始化本质是用静态数据（编译时已知）为数组初始化。此时数组的长度由静态数据的个数决定。一维数组声明和静态初始化有两个格式：
